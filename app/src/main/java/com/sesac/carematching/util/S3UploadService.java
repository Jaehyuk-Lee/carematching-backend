...
public class S3UploadService {
    ...
    public String saveCaregiverImageFile(MultipartFile multipartFile) throws IOException {
        // 파일 타입 검증: 이미지 파일만 허용
        String contentType = multipartFile.getContentType();
        if (contentType == null || !contentType.startsWith("image/")) {
            throw new IllegalArgumentException("업로드된 파일은 이미지가 아닙니다.");
        ...
        // UUID와 원본 파일명을 조합하고 "community_image/" 폴더를 prefix로 사용
        String uniqueFileName = UUID.randomUUID().toString() + "_" + originalFilename;
        String fileKey = "caregiver_image/" + uniqueFileName;
    ...
    public void deleteCaregiverImageFile(String fileUrl) {
        if (fileUrl == null || fileUrl.isEmpty()) {
            return;
        }
